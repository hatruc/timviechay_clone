...
const XemNhieuNhat = ({ listXemNhieuNhat }: { listXemNhieuNhat: any; }) => {

    return (
        <div className={s.box_blog_9}>
            ...
            <div className={s.box_blog_body_2}>
                {listXemNhieuNhat?.map((item: any, index: any) => {
                    return (
                        <div key={index} className={s.box_item_1}>
                            <div className={s.box_body}>
                                <Image src={item?.new_picture || '/images/blog/trang-chu-blog/cam-nang-tim-viec.png'} alt={""} width={281} height={199} className={s.image_item_3}></Image>
                                <div className={s.info_list}>
                                    <a href="#" style={{ textDecoration: "none" }} className={s.title}>{item?.new_title || ''}</a>
                                    <span className={s.time}>{getDate(item?.time) || 'chưa cập nhật ngày'}</span>
                                </div>
                            </div>
                            {index != listXemNhieuNhat.length - 1 ? <div className={s.line}></div> : <></>}
                        </div>
                    )
                })}
