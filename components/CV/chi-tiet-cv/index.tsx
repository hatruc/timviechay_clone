...
const Detail_cv: React.FC<{ idcv: any; type: any; id: any; alias: any; in4CVSsr: any, dataCvMau: any }> = ({
  idcv,
  type,
  id,
  alias,
  ...
  const checkBox = (id: string) => {
    if ("menu" in html) {
      const box = html["menu"]?.filter((item: any) => item.id === id)[0];
      return box;
    }
  }

  const checkBlock = (id: string) => {
    if ("experiences" in html) {
      const block = html["experiences"]?.filter((item: any) => item.id === id)[0];
      return block;
    }
  }
  ...
  const [htmlStringDone, setHtmlStringDone] = useState(false)
  ...
  useEffect(() => {
    ...
    // setTimeout(() => {
    if (htmlStrings) {
      handleSetData(html, dataCvMau?.html_vi, false, type == "1");
    }
    // }, 1500)
  }, [htmlStrings])
              ...
              <div className="container_content_taocv">
                ...
                <div className="box_content_taocv" style={{ width: "790px" }}>
                  <div className="page_cv" data-nopagi="1">
                    <input type="hidden" id="cvid" name="cvid" value={1853} />
                    {htmlStrings && (
                      <div
