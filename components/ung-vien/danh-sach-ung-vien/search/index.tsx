...
const Search = ({
    ...
    const [selectJob, setSelectJob] = useState<number>();
    const [selectExp, setSelectExp] = useState<number>();
    const [selectSalary, setSelectSalary] = useState<number>();
    const [selectLevel, setSelectLevel] = useState(-1);
    const [selectWorkForm, setSelectWorkForm] = useState<number>();
    ...
    const changeParam = () => {
        ...
        if(searchParams.has('city')) {
            setSelectCity(Number(searchParams.get('city')))
            handleDistrict(Number(searchParams.get('city')))
        } else {
            onChangeCity(0)
        }
        if(searchParams.has('kinhNghiem')) {
            setSelectExp(Number(searchParams.get('kinhNghiem')))
        } 
        if(searchParams.has('mucLuong')) {
            setSelectSalary(Number(searchParams.get('mucLuong')))
        } 
        if(searchParams.has('capBac')) {
            setSelectLevel(Number(searchParams.get('capBac')))
        } 
        if(searchParams.has('hinhThuc')) {
            setSelectWorkForm(Number(searchParams.get('hinhThuc')))
        } 
        if(searchParams.has('catid')) {
            setSelectJob(Number(searchParams.get('catid')))
        }  
        if(searchParams.has('district')) {
            setSelectDistrict(Number(searchParams.get('district')))
        }  
                ...
                <div className={s.body}>
                    ...
                    <div className={s.item_2}>
                        ...
                        <div className={s.advanced_filtering} onClick={handleFilter}>
                            ...
                            Lọc nâng cao
                            {filter == true ? <Image src={"/images/nha-tuyen-dung/danh-sach-tin-tuyen-dung/up.svg"} alt={""} height={20} width={20} style={{ height: "20px", width: "20px" }}></Image> :
                                <Image src={"/images/nha-tuyen-dung/danh-sach-tin-tuyen-dung/down.svg"} alt={""} height={20} width={20} style={{ height: "20px", width: "20px" }}></Image>}
                        </div>
                    ...
                    {filter == true ? <div className={s.item_3}>
                        ...
                        {showDistrict && listDistrict.length > 0 ?
                            <div className={s.box_search_5}>
