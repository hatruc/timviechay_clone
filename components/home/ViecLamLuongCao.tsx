...
const ViecLamLuongCao = ({ data, changeChooseAuto }: any) => {
    const [currenPage, setCurrentPage] = useState<number>(0);
    // console.log(data)
    const totalItem = Math.ceil(data?.ViecLamThuongHieu.length / 15)
    ...
    return (
        <div className="box_wrap">
            <div className="wrapper_company pt60">
                <p className="job_hapdan"><span className="F8971C">VIỆC LÀM</span><span> LƯƠNG CAO</span></p>
                {/* <div className="filter_tag">
                    <select name="select-luongcao" className="select-luongcao" >
                    ...
                    <div className="box_list_tag">
                        <div className="show_tag show_left" onClick={handlePrevClick} style={{ display: activeIndex === 0 ? 'none' : 'block', stroke: '#3582CD' }}>
                        ...
                        <div className="list_tag">
                            {tags.map((tag, index) => (
                                <a
                                    ...
                                    onClick={() => {handleTagClick(index), changeChooseAuto(tag.id)}}
                                >
                                    {tag.label}
                                </a>
                            ))}
                        </div>
                        <div className="show_tag show_right" onClick={handleNextClick} style={{ display: activeIndex === tags.length - 1 ? 'none' : 'block' }}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
                                <rect x="0.5" y="0.5" width="27" height="27" rx="13.5" fill="white" />
                                <rect x="0.5" y="0.5" width="27" height="27" rx="13.5" stroke="#3582CD" />
                    ...
                    <div className='select-ngaunhien'>
                        <select name="select-random" className="select-random select-random-salary">
                    ...
                    <Carousel ref={ref} dots={false}>
                        {
                            divArray.map((item: any, index: any) => (
                                <div className="list_company" key={index}>
                                    {
                                        data?.ViecLamThuongHieu?.slice(index * 15, (index + 1) * 15).map((item: any, index: any) => {
                                            // console.log(item)
                ...
                {
                    data?.ViecLamThuongHieu.length > 0 &&
                    <Pagination handleNextClick={handleNextClick} handlePrevClick={handlePrevClick} totalItem={totalItem} handleGoToSlide={handleGoToSlide} />
                }
            </div>
        </div >
    )
...
export default ViecLamLuongCao
