...
import s from "./qly-danh-sach.module.scss";
...
const ItemDataRow: React.FC<ItemDataProps> = ({
  ...
  const [selectedftBox, setSelectedftBox] = useState<number>(0);
  ...
  return (
    <tr>
      <td>{stt}</td>
      <td>
        <p  className={s["t-xanh"]}>
          {data.use_name}
        </p>
        <Link href={`/chi-tiet-ung-vien/${createLinkTilte(data.use_name)}-${data.use_id}.html`}>
        <p className={s["t-xanh"]}>(Xem chi tiết)</p>
        </Link>
      </td>
      <td>
        {" "}
        <a href="#" className={s["t-xanh"]}>
          {data.new_title}
        </a>
      </td>
...
const ChuyenVienGui = () => {
  // ftBox
  const header = ["STT", "Vị trí tuyển dụng", "Vị trí", "Ngày nộp", "Hẹn phỏng vấn", "Ghi chú", "Kết quả"];
  ...
  const filterAfterDelete = (id: number) => {
    const filter = dataUser.filter(
      (item: any) => item.result !== selectedftBox
    );
    setDataUser(filter);
  };
  ...
  return (
    <Temp_comp>
      <>
        <Box_container title={"CHUYÊN VIÊN GỬI ỨNG VIÊN"}>
          <div className={s.filter_box}>
            <select
              className={s.ft_uvut + " " + s.ft1}
              value={selectedftBox}
              onChange={(e) => handleSelectChangeftBox(e)}
            >
            ...
            <table className={s.table} >
              <thead>
                <tr>
                  ...
                  <th style={{ width: "14%" }}>Ngày nộp</th>
                  <th style={{ width: "14%" }}>Hẹn phỏng vấn</th>
                  ...
                  dataUser.map((data: any, index: number) => {
                    return (
                      <ItemDataRow
                        ...
                        handleFilter={() => {}}
                        handleOpenModal={(e, text) => handleChooseModalOpen(e, {
                          text: data.note_uv,
                          use_id: data.id
                        })}
          ...
          {listUser.total > 0 && <Pagination_page total={listUser.total} />}
        </Box_container>
        <ModalDelete
