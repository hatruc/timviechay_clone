...
import { login, loginType } from '../api/handle_uv';
import { cookieAuth, cookieId, cookieLogin, cookieName, cookieRfToken, cookieToken, cookieType, setMultipleCookie, useLoading } from '@/components/service/functions';
...
const Register = () => {
    const { setAll, candiAllowSearch, setCandiAllowSearch, setCandiAllowEmployerSearch, changePercent,  url ,changeUrl  } = useContext(NTD_UV_Context)
    ...
    const { contextHolder, startMessage, stopMessage, handleError } = useLoading()
    ...
    const onSubmit = async (data: any) => {
        ...
        const result = await POST('user/LoginCandidate', reqData)
        if (result?.result) {
            ...
            setMultipleCookie(
                `${result?.data?.Token}`,
                `${result?.data?.data?.use_id}`,
                `${result?.data?.data?.auth}`,
                `${result?.data?.data?.type}`,
                `${result?.data?.data?.userName}`,
                `${result?.data?.data?.use_phone_tk}`,
                `${result?.data?.data?.use_logo}`,
                `${result?.data?.RefreshToken}`,
                `${result?.data?.data?.percentHoSo}`,
            )
            setAll(`${result?.data?.data?.userName}`, `${result?.data?.data?.use_phone_tk}`, `${result?.data?.data?.use_logo}`)
            ...
            changeUrl('')
            // console.log('>>> check candiallowemployersearch: ', result?.data?.data?.usc_search);
                ...
                <div className={styles.wrapper_body}>
                    <div className={styles.wrapper_content}>
                        ...
                        <form className={styles.container_right} onSubmit={handleSubmit(onSubmit)}>
                            <p className={styles.title_right}>TIMVIECHAY.VN - VIỆC TRONG TẦM TAY, SĂN NGAY KẺO LỠ</p>
                            ...
                            <div className={styles.box_confirm}>
                                ...
                                <button className={styles.btn_confirm} type="submit" disabled={isBtnDisabled}>
                                    ĐĂNG NHẬP
                                </button>
                                <div className={styles.btn_content}>
                                    <p>Bạn chưa có tài khoản?</p>
                                    <Link href={'/dang-ky-ung-vien'}>ĐĂNG KÝ NGAY</Link>
                                </div>
                            </div>
                        </form>
                    </div>
