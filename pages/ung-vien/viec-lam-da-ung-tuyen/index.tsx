...
import { getDanhGiaUngVien, getDate, getTokenServerSide } from "@/functions/functions";
...
export const getServerSideProps = async (context: NextPageContext) => {
  ...
  const result = await POST_SERVER(`${process.env.NEXT_PUBLIC_BASE_URL_API_CANDI}/candidate/JobApply`, { page: 1, pageSize: 10 }, token)
  if (result?.result) {
    appliedJob = result?.data
    total = result?.total
  }
...
interface DataType {
  ...
  usc_id: string | number,
  link: string,
  id: string | number,
}

const ViecLamDaUngTuyen: NextPage<pageProps> = ({ preData, preTotal }) => {
    ...
    if (Array.isArray(data)) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const tbldata: DataType = {
          ...
          deadline: getDate(element?.new_han_nop),
          date: getDate(element?.nhs_time),
  ...
  const fetchData = async (_page: number, _perPage: number) => {
    const result = await POST(`candidate/JobApply`, { page: _page, pageSize: _perPage })
    if (result?.result) {
  ...
  const unapplyJob = async (id: number, name: string = '') => {
    if (confirm(`Bạn có muốn hủy ứng tuyển ${name}?`)) {
      // alert(`${id}, ${name}`)
      const result = await POST(`candidate/UnApplyJob`, {id: id})
      if (result?.result) {
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      align: "center",
      render: (record) => (
        <div>{getDanhGiaUngVien(record)}</div>
      )
    },
    {
      title: 'Xóa',
      ...
      align: "center",
      render: (record) => (
        <div style={{ color: "#FF0707" }} onClick={() => unapplyJob(record.new_id, record.position)}>
