...
;(function (global, factory) {
	...
	var EVENT_POINTER_UP = WINDOW.PointerEvent
		...
		limitCropBox: function limitCropBox(sizeLimited, positionLimited) {
			...
			if (positionLimited) {
				if (limited) {
					...
					cropBoxData.maxLeft =
						Math.min(containerData.width, canvasData.left + canvasData.width) - cropBoxData.width
					cropBoxData.maxTop =
						Math.min(containerData.height, canvasData.top + canvasData.height) - cropBoxData.height
				} else {
		...
		renderCropBox: function renderCropBox() {
			...
			if (options.movable && options.cropBoxMovable) {
				...
				setData(
					this.face,
					DATA_ACTION,
					cropBoxData.width >= containerData.width && cropBoxData.height >= containerData.height
						? ACTION_MOVE
						: ACTION_ALL
				)
			}
	...
	var change = {
		change: function change(e) {
			...
			if (this.limited) {
				...
				maxWidth =
					minLeft +
					Math.min(containerData.width, canvasData.width, canvasData.left + canvasData.width)
				maxHeight =
					minTop +
					Math.min(containerData.height, canvasData.height, canvasData.top + canvasData.height)
			}
			...
			switch (action) {
				...
				case ACTION_EAST:
					if (
						range.x >= 0 &&
						(right >= maxWidth || (aspectRatio && (top <= minTop || bottom >= maxHeight)))
					) {
						renderable = false
						break
					}
				...
				case ACTION_NORTH:
					if (
						range.y <= 0 &&
						(top <= minTop || (aspectRatio && (left <= minLeft || right >= maxWidth)))
					) {
						renderable = false
						break
					}
				...
				case ACTION_WEST:
					if (
						range.x <= 0 &&
						(left <= minLeft || (aspectRatio && (top <= minTop || bottom >= maxHeight)))
					) {
						renderable = false
						break
					}
				...
				case ACTION_SOUTH:
					if (
						range.y >= 0 &&
						(bottom >= maxHeight || (aspectRatio && (left <= minLeft || right >= maxWidth)))
					) {
						renderable = false
						break
					}
		...
		zoomTo: function zoomTo(ratio, pivot, _originalEvent) {
			...
			if (ratio >= 0 && this.ready && !this.disabled && options.zoomable) {
				...
				if (_originalEvent) {
					...
					canvasData.left -=
						(newWidth - width) * ((center.pageX - offset.left - canvasData.left) / width)
					canvasData.top -=
						(newHeight - height) * ((center.pageY - offset.top - canvasData.top) / height)
