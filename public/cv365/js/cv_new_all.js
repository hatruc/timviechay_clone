...
function check_cv_content() {
  ...
  if (selector.length) {
    selector.each(function () {
      ...
      // console.log('content_suggest: ', data_box)
      if (content_suggest) {
        //check nội dung box
        if ($(this).hasClass('box-content')) {
          // console.log($(this).hasClass('box-content'), '=======================')
          if (
            (replaceStr(content_suggest.content) == replaceStr($(this).html()) && replaceStr(content_suggest.content) != '') ||
            replaceStr($(this).html().toLowerCase()).includes('cv365')
          ) {
...
function changeLayoutCv() {
  ...
  if (!$('#form-cv .cv_page').length) {
    let htmlTop = $('#cv-top')[0] && !$('#cv-top').parents('.all').length ? $('#cv-top')[0].outerHTML : '',
      htmlFooter = $('#form-cv .footer')[0] ? $('#form-cv .footer')[0].outerHTML : '',
...
function paginationCv(page) {
  try {
    ...
    if (heightFreeLeft > heightFreeRight) {
      page_current.find(`${cv_left}`).css('padding-bottom', heightFreeLeft)
    } else {
      if (heightFreeRight > 0) {
        page_current.find(`${cv_right}`).css('padding-bottom', heightFreeRight)
      }
    }
...
function addWatermark() {
  let html = `<div class="watermark_js">© Timviec365.vn</div>`
  let totalPage = $('.cv_page').length
  for (let page = 1; page <= totalPage; page++) {
    let currentPage = $(`.cv_page[data-page="${page}"]`)
...
function changeBackgroundCv(url_image) {
  if (url_image) {
    ...
    if (
      ...
      $('.box-content, .skill-name , #cv-boxtitle').css('color', '#000')
      $('.bar-exp').css('background')
    } else {
      $('div#cv-boxtitle,.box-content,#cv-content .head div,#block01 .exp-date,.exp-content,.exp-subtitle').css('color', '#000')
    }
...
function unChangeBackgroundCv() {
  ...
  if (
    ...
    $(
      '#cv-top,.footer,.all,.cvo-block,#cv-profile-job,#cv-main,#cv-right,#cv-boxtitle,#cv-right h3,#block01 .exp-date,div#prof,.cvo-block, #cv-top h2'
    ).css('background', 'none')
    $('.block-title').removeAttr('style')
  } else {
    $(
      '#cv-top,.footer,.all,.cvo-block,#cv-profile-job,#cv-main,#cv-right,#cv-boxtitle,#cv-right h3,#block01 .exp-date,div#prof,.cvo-block, #cv-top h2,#cv-content .head'
    ).removeAttr('style')
  }
