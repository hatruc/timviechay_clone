...
const cookieToken = "work247_token";
const cookieRfToken = "rf_token";
const cookiePhone = "phone";
const cookieType = "work247_type";
const cookieStep1 = "isStep1Ok";
const cookieTempId = "tempId";
const cookieId = "id";
const cookieAuth = "auth";
const cookieForgetPass = "isForgetPass";
const cookieOTP = "isOTP";
const cookieLogin = "isLogin";
const cookieName = "userName";
const cookieLogo = "logo";
const cookieCandiAllowSearch = "candiAllowSearch";
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.data && data.data.result) {
        ...
        downloadAsPDF(`data:application/octet-stream;base64,${data.data.file}`);
      }
    },
    error: (err) => {
      console.log(err);
  ...
  $(document).on("click", "#dk-btn", (t) => {
    ...
    if (isValid) {
      ...
      var phone = $("#mobile").val(),
        ...
        username = $("#cv-profile-fullname").text(),
        jobName = $("#cv-profile-job").text(),
        address = $("#cv-profile-address").text(),
      ...
      $.ajax({
        ...
        success: function (respons) {
          ...
          if (respons.data && respons.data.result == true) {
            ...
            downloadAsPDF(
              `https://timviechay.vn/download/cv_pdf/user_${user_id}/cvid_${idcv}/${idcv}-timviechay`
            );
            window.location.href = "/";
            Cookies.remove(isStep1Ok);
...
function xemtruoc_cv() {
  let id = window.localStorage.getItem("_DEVICEID_");
  var sortAbleArea = [
    { el: ".sortable", item: ".block", area: "menu" },
    { el: ".sort_block", item: ".cvo-block", area: "experiences" },
  ];

  const json_cv = $.exportData();
  ...
  $.ajax({
    cache: false,
    type: "POST",
    url: "https://api.timviec365.vn/api/timviec/cv/renderPreview",
    data: { json_cv, lang, id, cvid, avatar },
    dataType: "JSON",
    beforeSend: function (response) { },
    success: function (res) {
      if (res.data && res.data.result) {
        let img_val = res.data.image;
        $("#cv_mau_new .img_cv").attr(
          "src",
          `data:image/png;base64,${img_val}`
        );
        $("#cv_mau_new").show();
      } else {
...
$(".btn_login_cv").click(function (e) {
  ...
  if (ccemail.val() == "") {
    ...
    if (ccemail.val().length > 0) {
      if (validateUser(ccemail.val()) == false) {
        if ($("#user_phone_error").hasClass("error") == false) {
          ccemail.after(
            '<label id="user_phone_error" class="error" for="user_phone">Vui lòng nhập đúng định dạng email hoặc số điện thoại.</label>'
          );
          ccemail.focus();
          ccemail.addClass("error");
        }
      ...
      success: function (data) {
        if (data?.data?.result) {
          ...
          $.ajax({
            ...
            success(response) {
              if (response?.data?.result) {
                ...
                alert("CV của bạn đã được gửi về Chat365");
                // open chat
                $(".bg-spinner").remove();
                var url = getCookie("url");
                if (isMac) {
          ...
          $(".auth_form .hrdot").html(
            '<label id="idchecklogin" class="error" for="idchecklogin">Tài khoản,mật khẩu của bạn không đúng hoặc email chưa đăng ký</label>'
          );
        }
      },
      error: function (err) {
...
function resg_new() {
  ...
  var cv_title = $("#cv-profile-job").text(),
    phone = $("#cv-profile-phone").text().trim(),
    email_lh = $("#cv-profile-email").text(),
    address = $("#cv-profile-address").text(),
    name = $("#cv-profile-fullname").text(),
    birthday = $("#cv-profile-birthday").text();
  var err = 2;
  if ("" != $("#cvid").val()) {
    if (
  ...
  if (err != "" && isStep1Ok != "1" && cookieIsLogin != "true") {
    if (!token) {
      $(window).scrollTop(0);
      $(window).scrollLeft(0);
    ...
    if (token) {
      $.ajax({
        ...
        data: {
          ...
          lang,
          allowSendChat: 1,
          // name_img: $('#cvo-profile-avatar').attr('src'),
          name_img: "u_cv_" + Math.floor(Date.now() / 1000),
          name_img_hide: "u_cv_" + Math.floor(Date.now() / 1000) + "_h",
        },
        dataType: "json",
